...
public class BubbleView extends View {
	...
	public void initRandomly(Circle c) {
		...
		c.x = (float) ((2 * Math.random() - 1) * halfWidth * 0.75f + halfWidth);
		c.y = getHeight() + (float) (Math.random() * MAX_BELOW);
		c.speed = (float) (Math.random() * SPEED_DIFF + MIN_SPEED);
		c.color = Color.argb((int) (Math.random() * 255), (int) (Math.random() * 255), (int) (Math.random() * 255), (int) (Math.random() * 255));
